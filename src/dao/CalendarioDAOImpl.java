...
public class CalendarioDAOImpl implements CalendarioDAO {
	 ...
	 * l'edizione viene individuata in base al idEdizione se l'edizione non è
...
// c'era un set secondo noi errato
	 ...
	 * viene individuata in base al idEdizione se l'edizione non è presente si
...
//		ps.setInt(1, idEdizione);
	...
	public ArrayList<Edizione> select(java.util.Date da, java.util.Date a) throws SQLException {
		...
		PreparedStatement ps = conn
				.prepareStatement("select * from calendario where CAST(dataInizio AS DATE) between '?' AND '?'");
	...
	public ArrayList<Edizione> select(int idCategoria, boolean future) throws SQLException {
		...
		if (future == true) {
			PreparedStatement ps = conn.prepareStatement(
					"select * from categoria cat join catalogo co on (cat.id_categoria=co.id_categoria) join calendario cal on (cal.id_corso=co.id_corso) where dataInizio = (select curdate());");
			ResultSet rs = ps.executeQuery();
			while (rs.next()) {
				int idEdizione = rs.getInt("id_edizione");
				int idCorso = rs.getInt("id_corso");
			...
			PreparedStatement ps = conn.prepareStatement(
					"select * from categoria cat join catalogo co on (cat.id_categoria=co.id_categoria) join calendario cal on (cal.id_corso=co.id_corso);");
			ResultSet rs = ps.executeQuery();
			while (rs.next()) {
				int idEdizione = rs.getInt("id_edizione");
				int idCorso = rs.getInt("id_corso");
	...
	public ArrayList<Edizione> select(boolean future) throws SQLException {
		...
		if (future == true) {
			PreparedStatement ps = conn
					.prepareStatement("select * from calendario where dataInizio = (select curdate());");
			ResultSet rs = ps.executeQuery();
			while (rs.next()) {
				int idEdizione = rs.getInt("id_edizione");
				int idCorso = rs.getInt("id_corso");
	...
	public ArrayList<Edizione> select(String idUtente, boolean future) throws SQLException {
		...
		if (future == true) {
			PreparedStatement ps = conn.prepareStatement(
					"SELECT * FROM calendario join iscritti using (id_edizione) join registrati using (id_utente =?) where dataInizio = (select curdate());");
			ResultSet rs = ps.executeQuery();
			while (rs.next()) {
				int idEdizione = rs.getInt("id_edizione");
				int idCorso = rs.getInt("id_corso");
